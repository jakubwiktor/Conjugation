...
out = findCellsWithSpots(expInfoObj,'Pos12','fluor_YFP_venus')
...
for vi = 3%:length(expInfoObj.imRange{pos_index, 2})
    ...
    transFluoIm2 = transFluoIm - imgaussfilt(transFluoIm,13);
    ...
    parfor sp = 1:length(xx)
        ...
        tmp = transFluoIm2(yy(sp)-gauss_kren_size : yy(sp)+gauss_kren_size ,...
            xx(sp)-gauss_kren_size : xx(sp)+gauss_kren_size);
        focisignal{sp} = mean(tmp(:));
    end
    
    keep_spots = [keep_spots{:}];
    sigmasX = [sigmasX{:}];
    sigmasY = [sigmasY{:}];
    focisignal = [focisignal{:}];
%     focisignal = double(focisignal) - double(median(transFluoIm(~seg)));
    
    sel_condition = keep_spots > 0.5 & sigmasX < 1+2*gauss_kren_size & sigmasY < 1+2*gauss_kren_size;
    
    figure
    imshow(transFluoIm,prctile(double(transFluoIm(:)),[0, 99.9]))
    hold on
%     plot(xx,yy,'rx')
    
    plot(xx(focisignal>1.5*mean(focisignal)),yy(focisignal>1.5*mean(focisignal)),'go')
